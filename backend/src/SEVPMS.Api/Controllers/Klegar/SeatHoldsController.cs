...
[ApiController]
[Authorize]
public sealed class SeatHoldsController(
    ISeatService seats,
    ...
    KlegarAuthorizationService authorization) : ControllerBase
{
    ...
    public async Task<ActionResult<SeatHoldResponse>> Hold(
        Guid eventId,
        ...
        CancellationToken ct)
    {
        ...
        var result = await seats.HoldAsync(eventId, userId, request, ct);
        ...
        CancellationToken ct)
    {
        ...
        if (!await authorization.CanManageBookingAsync(HttpContext, bookingId, null, ct)) return Forbid();
        return await seats.CommitHoldAsync(holdToken, customerUserId, bookingId, ct)
            ? Ok(new { committed = true, bookingId })
            : Conflict(new { committed = false, error = "hold_invalid_or_expired" });
    }
}
