...
public sealed class PaymentService(
    ...
    : IPaymentService
{
    public async Task<IReadOnlyList<PaymentResponse>> GetMineAsync(Guid customerUserId, CancellationToken cancellationToken = default)
        => (await paymentRepository.GetByCustomerAsync(customerUserId, cancellationToken)).Select(Map).ToList();

    public async Task<PaymentResponse> StartAsync(Guid customerUserId, StartPaymentRequest request, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<PaymentResponse> CompleteMockAsync(Guid paymentId, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<PaymentResponse> FailMockAsync(Guid paymentId, CancellationToken cancellationToken = default)
    {
        var payment = await paymentRepository.GetByIdAsync(paymentId, cancellationToken)
            ?? throw new KeyNotFoundException("Payment was not found.");

        if (payment.Status == PaymentStatus.Successful)
            throw new InvalidOperationException("Successful payments cannot be marked failed.");
